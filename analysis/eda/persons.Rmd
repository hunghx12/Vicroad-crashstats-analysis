...
summary(persons %>%
          ...
          dplyr::select(HELMET_BELT_WORN)
  )
```

The variable 'HELMET_BELT_WORN' is supposed to be a boolean variable or factor indicating whether the person invovled in the accidents was wearing seat belt/ helmet or not, but in the data set it is unjustifiable. Hence, we cannot use it to analyse/ build prediction model.
...
persons_filtered <- persons %>% 
    filter(Inj.Level.Desc != "Unknown" & SEATING_POSITION != "PS" & 
...
We will try to fit a model to see if there is connection between seating positions, age group and injurious level.
...
- For individuals within the same age group, if they are seated in the 'Passenger Left' position, the odds of being injured are 4.9 times higher than for drivers, while the odds of sustaining injuries in other seating positions in the car are relatively the same. \
- For people seated in the same position in a car, the odds of sustaining injuries for those in the age groups 5-12 and 16-17 are 2.46 and 1.77 times higher, respectively, compared to people in the age group 0-4. For other age groups, the odds are nearly the same. \